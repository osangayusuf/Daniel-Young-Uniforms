            ...
            @foreach($sub_categories as $sub)
                <li>
                    <div class="px-6 py-3 rounded-lg mx-1 hover:bg-light-purple hover:text-whiteish relative">
                        <a href="{{ route('admin.products') . '?category=' . $current_category . '&sub=' . $sub->sub_category }}" class="block w-4/5 h-full">
                            {{$sub->sub_category}}
                        </a>
                    ...
                    <ul id="dropdown" class="hidden">
                        @foreach($sub->classification as $class)
                            <li class="text-xs md:text-sm mx-4">
                                <a href="{{ route('admin.products') . '?category=' . $current_category . '&sub=' . $sub->sub_category . '&class=' . $class->classification }}" class="block px-6 py-2 rounded-lg mx-1 hover:bg-light-purple hover:text-whiteish">
