    ...
    <div class="w-[97%] mx-auto">
        ...
        <div class="w-11/12 mt-16 ml-4">
            ...
            <div class="flex items-center text-xs md:text-sm">
                ...
                <p class="inline mx-2 self-center">Shop</p>
            </div>
            @if(!is_null($current_category))
                <h1 class="text-dark-blue font-bold lg:hidden my-8">{{ \App\Models\Category::whereId($current_category)->first()->name }}</h1>
            @endif
            ...
            <div class="flex lg:hidden mt-10 mb-16">
                <div class="flex items-center gap-5 relative basis-3/5">
                    ...
                    <ul id="shop-categories-menu" class="absolute top-6 left-1 rounded-md shadow-xl pt-10 pb-4 bg-whiteish hidden z-10 space-y-0.5 w-3/4">
                        <li>
                            <a href="{{ route('shop') . '?category=' . $current_category }}"  class="block px-6 py-3 rounded-lg mx-1 hover:bg-light-purple hover:text-whiteish
                                @if(Request::fullUrl() == ((route('shop') . '?category=' . $current_category)))
                        ...
                        @if(!is_null($current_category))
                            ...
                            <li>
                                <a href="{{ route('shop') . '?category=' . $current_category . '&sub=' . $sub->sub_category }}"  class="block px-6 py-3 rounded-lg mx-1 hover:bg-light-purple hover:text-whiteish
                                    @if(Request::fullUrl() == ((route('shop') . '?category=' . $current_category. '&sub=' . $sub->sub_category)))
                                        bg-dark-blue text-white
                                    @endif">
                                    {{ $sub->sub_category }}
                                </a>
                ...
                {{--      FILTER      --}}
                @if(!is_null($current_category))
                    <div class="text-right flex justify-end relative items-center gap-2 basis-2/5 text-sm">
                        <button id="shop-filter-button" class="text-xs">
                            Filter
                        ...
                        <ul id="shop-filter-menu" class="absolute top-6 right-0 rounded-md shadow-xl bg-whiteish text-left z-10 py-3 hidden min-w-44 space-y-0.5">
                            <li class="">
                                @if(isset($current_sub_category))
                                    <a href="{{ route('shop') . '?category=' . $current_category . '?sub=' . $current_sub_category }}" class="block px-6 py-3 rounded-lg mx-1 hover:bg-light-purple hover:text-whiteish">All</a>
                                @else
                            ...
                            @if(isset($current_sub_category))
                                @foreach($sub_categories->where('sub_category', $current_sub_category)->first()->classification as $class)
                                    <li class="">
                                        <a href="{{ route('shop') . '?category=' . $current_category . '&sub=' . $current_sub_category . '&class=' . $class->classification }}" class="block px-6 py-3 rounded-lg mx-1 hover:bg-light-purple hover:text-whiteish">
            ...
            <div class="flex flex-row gap-4 mt-3 lg:mt-16">
                <x-shop-sidebar :sub_categories='$sub_categories' :current_category='$current_category'></x-shop-sidebar>
                <!-- ITEM SHOWCASE -->
